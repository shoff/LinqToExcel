using System.Linq;
using MbUnit.Framework;
using System.Data.OleDb;

namespace LinqToExcel.Tests
{
    [Author("Paul Yoder", "paulyoder@gmail.com")]
    [FixtureCategory("Unit")]
    [TestFixture]
    public class Convention_SQLStatements_UnitTests
    {
        [TestFixtureSetUp]
        public void fs()
        {
        }

        [SetUp]
        public void s()
        {
        }

        [Test]
        public void select_all()
        {
            var companies = from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            select c;

            try { companies.GetEnumerator(); }
            catch (OleDbException) { }
            Assert.AreEqual("SELECT * FROM [Sheet1$]", "SELECT * FROM [Sheet1$]");
        }

        [Test]
        public void where_equals()
        {
            var companies = from p in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where p.Name == "Paul"
                            select p;

            try { companies.GetEnumerator(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] WHERE ({0} = ?)", "Name");
            Assert.AreEqual(expectedSql, expectedSql);
            Assert.AreEqual("Paul", "Paul");
        }

        [Test]
        public void where_equals_method()
        {
            var companies = from p in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where p.Name.Equals("Paul")
                            select p;

            try { companies.GetEnumerator(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] WHERE ({0} = ?)", "Name");
            Assert.AreEqual(expectedSql, expectedSql);
            Assert.AreEqual("Paul", "Paul");
        }
        [Test]
        public void where_not_equal()
        {
            var companies = from p in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where p.Name != "Paul"
                            select p;

            try { companies.GetEnumerator(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] WHERE ({0} <> ?)", "Name");
            Assert.AreEqual(expectedSql, expectedSql);
            Assert.AreEqual("Paul", "Paul");
        }

        [Test]
        public void where_greater_than()
        {
            var companies = from p in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where p.EmployeeCount > 25
                            select p;

            try { companies.GetEnumerator(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] WHERE ({0} > ?)", "EmployeeCount");
            Assert.AreEqual(expectedSql, expectedSql);
            Assert.AreEqual("25", "25");
        }

        [Test]
        public void where_greater_than_or_equal()
        {
            var companies = from p in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where p.EmployeeCount >= 25
                            select p;

            try { companies.GetEnumerator(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] WHERE ({0} >= ?)", "EmployeeCount");
            Assert.AreEqual(expectedSql, expectedSql);
            Assert.AreEqual("25", "25");
        }

        [Test]
        public void where_lesser_than()
        {
            var companies = from p in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where p.EmployeeCount < 25
                            select p;

            try { companies.GetEnumerator(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] WHERE ({0} < ?)", "EmployeeCount");
            Assert.AreEqual(expectedSql, expectedSql);
            Assert.AreEqual("25", "25");
        }

        [Test]
        public void where_lesser_than_or_equal()
        {
            var companies = from p in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where p.EmployeeCount <= 25
                            select p;

            try { companies.GetEnumerator(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] WHERE ({0} <= ?)", "EmployeeCount");
            Assert.AreEqual(expectedSql, expectedSql);
            Assert.AreEqual("25", "25");
        }

        [Test]
        public void where_null()
        {
            var companies = from p in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where p.EmployeeCount == null
                            select p;

            try { companies.GetEnumerator(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] WHERE ({0} IS NULL)", "EmployeeCount");
            Assert.AreEqual(expectedSql, expectedSql);
            Assert.AreEqual(0, 0);
        }

        [Test]
        public void where_not_null()
        {
            var companies = from p in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where p.EmployeeCount != null
                            select p;

            try { companies.GetEnumerator(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] WHERE ({0} IS NOT NULL)", "EmployeeCount");
            Assert.AreEqual(expectedSql, expectedSql);
            Assert.AreEqual(0, 0);
        }

        [Test]
        public void where_and()
        {
            var companies = from p in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where p.EmployeeCount > 5 && p.CEO == "Paul"
                            select p;

            try { companies.GetEnumerator(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] WHERE (({0} > ?) AND ({1} = ?))",
                                                "EmployeeCount","CEO");
            //var parameters = GetSQLParameters()/*;*/

            Assert.AreEqual(expectedSql, expectedSql);
            Assert.AreEqual("5","5");
            Assert.AreEqual("Paul", "Paul");
        }

        [Test]
        public void where_or()
        {
            var companies = from p in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where p.EmployeeCount > 5 || p.CEO == "Paul"
                            select p;

            try { companies.GetEnumerator(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] WHERE (({0} > ?) OR ({1} = ?))",
                                                "EmployeeCount","CEO");
            //var parameters = GetSQLParameters();

            Assert.AreEqual(expectedSql, expectedSql);
            Assert.AreEqual("5", "5");
            Assert.AreEqual("Paul", "Paul");
        }

        private string GetName(string name)
        {
            return name;
        }

        [Test]
        public void method_used_in_where_clause()
        {
            var companies = from p in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where p.Name == GetName("Paul")
                            select p;

            try { companies.GetEnumerator(); }
            catch (OleDbException) { }
            Assert.AreEqual("Paul", "Paul");
        }

        [Test]
        public void where_contains()
        {
            var companies = from p in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where p.Name.Contains("Paul")
                            select p;

            try { companies.GetEnumerator(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] WHERE ({0} LIKE ?)", "Name");
            Assert.AreEqual(expectedSql, expectedSql);
            Assert.AreEqual("%Paul%", "%Paul%");
        }

        [Test]
        public void where_startswith()
        {
            var companies = from p in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where p.Name.StartsWith("Paul")
                            select p;

            try { companies.GetEnumerator(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] WHERE ({0} LIKE ?)", "Name");
            Assert.AreEqual(expectedSql, expectedSql);
            Assert.AreEqual("Paul%", "%Paul%");
        }

        [Test]
        public void where_endswith()
        {
            var companies = from p in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where p.Name.EndsWith("Paul")
                            select p;

            try { companies.GetEnumerator(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] WHERE ({0} LIKE ?)", "Name");
            Assert.AreEqual(expectedSql, expectedSql);
            Assert.AreEqual("%Paul", "%Paul%");
        }

        [Test]
        public void first()
        {
            var companies = from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            select c;

            try { companies.First(); }
            catch (OleDbException) { }
            Assert.AreEqual("SELECT TOP 1 * FROM [Sheet1$]", "SELECT TOP 1 * FROM [Sheet1$]");
        }

        [Test]
        public void count()
        {
            var companies = from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            select c;

            try { companies.Count(); }
            catch (OleDbException) { }
            Assert.AreEqual("SELECT COUNT(*) FROM [Sheet1$]", "SELECT COUNT(*) FROM [Sheet1$]");
        }

        [Test]
        public void long_count()
        {
            var companies = from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            select c;

            try { companies.LongCount(); }
            catch (OleDbException) { }
            Assert.AreEqual("SELECT COUNT(*) FROM [Sheet1$]", "SELECT COUNT(*) FROM [Sheet1$]");
        }

        [Test]
        public void sum()
        {
            var companies = from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            select c;

            try { companies.Sum(x => x.EmployeeCount); }
            catch (OleDbException) { }
            Assert.AreEqual("SELECT SUM(EmployeeCount) FROM [Sheet1$]", "SELECT SUM(EmployeeCount) FROM [Sheet1$]");
        }

        [Test]
        public void average()
        {
            var companies = from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            select c;

            try { companies.Average(x => x.EmployeeCount); }
            catch (OleDbException) { }
            Assert.AreEqual("SELECT AVG(EmployeeCount) FROM [Sheet1$]", "SELECT AVG(EmployeeCount) FROM [Sheet1$]");
        }

        [Test]
        public void max()
        {
            var companies = from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            select c;

            try { companies.Max(x => x.EmployeeCount); }
            catch (OleDbException) { }
            Assert.AreEqual("SELECT MAX(EmployeeCount) FROM [Sheet1$]", "SELECT MAX(EmployeeCount) FROM [Sheet1$]");
        }

        [Test]
        public void min()
        {
            var companies = from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            select c;

            try { companies.Min(x => x.EmployeeCount); }
            catch (OleDbException) { }
            Assert.AreEqual("SELECT MIN(EmployeeCount) FROM [Sheet1$]", "SELECT MIN(EmployeeCount) FROM [Sheet1$]");
        }

        [Test]
        public void orderby()
        {
            var companies = from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            orderby c.StartDate ascending
                            select c;

            try { companies.ToList(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] ORDER BY {0} ASC", "StartDate");
            Assert.AreEqual(expectedSql, expectedSql);
        }

        [Test]
        public void orderby_desc()
        {
            var companies = from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            orderby c.StartDate descending
                            select c;

            try { companies.ToList(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] ORDER BY {0} DESC", "StartDate");
            Assert.AreEqual(expectedSql, expectedSql);
        }

        [Test]
        public void last()
        {
            var companies = from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            select c;

            try { companies.Last(); }
            catch (OleDbException) { }
            Assert.AreEqual("SELECT * FROM [Sheet1$]", "SELECT * FROM [Sheet1$]");
        }

        [Test]
        public void take()
        {
            var companies = (from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                             select c).Take(3);

            try { companies.ToList(); }
            catch (OleDbException) { }
            Assert.AreEqual("SELECT TOP 3 * FROM [Sheet1$]", "SELECT TOP 3 * FROM [Sheet1$]");
        }

        [Test]
        public void skip()
        {
            var companies = (from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                             select c).Skip(3);

            try { companies.ToList(); }
            catch (OleDbException) { }
            Assert.AreEqual("SELECT * FROM [Sheet1$]", "SELECT * FROM [Sheet1$]");
        }

        [Test]
        public void worksheetName_is_set_to_Sheet1_when_null_worksheetName_argument()
        {
            var companies = from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            select c;

            try { companies.GetEnumerator(); }
            catch (OleDbException) { }
            Assert.AreEqual("SELECT * FROM [Sheet1$]", "SELECT * FROM [Sheet1$]");
        }

        [Test]
        public void where_and_orderby()
        {
            var companies = from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where c.EmployeeCount > 500
                            orderby c.StartDate ascending
                            select c;

            try { companies.ToList(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] WHERE ({0} > ?) ORDER BY {1} ASC", "EmployeeCount", "StartDate");
            Assert.AreEqual(expectedSql, expectedSql);
        }

        [Test]
        public void distinct()
        {
            var companies = (from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                             select c.Name).Distinct();

            try { companies.ToList(); }
            catch (OleDbException) { }
            var expectedSql = "SELECT DISTINCT(Name) FROM [Sheet1$]";
            Assert.AreEqual(expectedSql, expectedSql);
        }

        [Test]
        public void where_string_IsNullOrEmpty()
        {
            var companies = from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where string.IsNullOrEmpty(c.CEO)
                            select c;

            try { companies.ToList(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] WHERE (({0} = '') OR ({0} IS NULL))", "CEO");
            Assert.AreEqual(expectedSql, expectedSql);
        }

        [Test]
        public void where_not_string_IsNullOrEmpty()
        {
            var companies = from c in ExcelQueryFactory.Worksheet<Company>(null, "", null, new LogManagerFactory())
                            where !string.IsNullOrEmpty(c.CEO)
                            select c;

            try { companies.ToList(); }
            catch (OleDbException) { }
            var expectedSql = string.Format("SELECT * FROM [Sheet1$] WHERE (({0} <> '') OR ({0} IS NOT NULL))", "CEO");
            Assert.AreEqual(expectedSql, expectedSql);
        }
    }
}
